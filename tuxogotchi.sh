...
draw_tux() {
    ...
    [ -n "$hat" ] && echo -e "${MAGENTA}       $hat${RESET}"
    echo -e "${YELLOW}     //   \\ \\${RESET}"
    echo -e "${YELLOW}    (|     | )${RESET}"
...
shop() {
    ...
    echo -e "2. Buy a hat for Tux"
    echo -e "3. Leave the shop"
    read -p "Choose an option (1/2/3): " shop_choice
    ...
    case $shop_choice in
        1)
            ...
            ;;
        2)
            echo -e "${YELLOW}=== Hats ===${RESET}"
            echo -e "1. Basic Hat (5 coins)"
            echo -e "2. Winter Cap (8 coins)"
            echo -e "3. Crown (15 coins)"
            read -p "Choose a hat (1/2/3): " hat_choice
            ...
            case $hat_choice in
                1)
                    if [ "$money" -ge 5 ]; then
                        hat="(\\_°-°_/) Basic Hat"
                        money=$((money - 5))
                        echo -e "${GREEN}You bought a Basic Hat for $tux_name!${RESET}"
                    else
                        echo -e "${RED}You don't have enough money!${RESET}"
                    fi
                    ;;
                2)
                    if [ "$money" -ge 8 ]; then
                        hat="(\\_°-°_/) Winter Cap"
                        money=$((money - 8))
                        echo -e "${GREEN}You bought a Winter Cap for $tux_name!${RESET}"
                    else
                        echo -e "${RED}You don't have enough money!${RESET}"
                    fi
                    ;;
                3)
                    if [ "$money" -ge 15 ]; then
                        ...
                        happiness=$((happiness + 2))
                        money=$((money - 15))
                        echo -e "${GREEN}You bought a Crown for $tux_name! Happiness increased!${RESET}"
                    else
                        echo -e "${RED}You don't have enough money!${RESET}"
                    fi
                    ;;
